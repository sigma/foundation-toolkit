    ...
    this.addDeleteButtons = function() {
        ...
        var hackedSubmit = function(event) {
            ...
            if ( imgId.match("delete_") ) {
                ...
                inForm = true;
            } else if (event.target.src && event.target.src.search("bouton_effacer.gif") != -1){
                event.stopPropagation();
                event.preventDefault();
                inForm = true;
            }
            ...
            if(inForm) {
                ...
                for (var i = allMsg.snapshotLength - 1; i>=0; i--) {
                    ...
                    if (realWindow.document.forms[0].elements[i+1].checked) {
                        ...
                        db.execute('BEGIN');
                        db.execute('INSERT INTO Messages_meta (date) VALUES (?)', [result]);
                        db.execute('INSERT INTO Messages_search (rowid, author, channel, body) ' +
